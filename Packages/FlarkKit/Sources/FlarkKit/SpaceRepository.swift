...
public actor SpaceRepository {
    ...
    public func bootstrap(spaceName: String) async throws {
        for dir in ["events", "profiles", "blobs", "events/\(identity.authorID)"] {
            try? await backend.makeDirectory(dir)
        }
        if !(try await backend.exists("flark.json")) {
            let manifest = ["schemaVersion": "1", "spaceID": spaceID, "name": spaceName]
            let data = try Self.encoder.encode(manifest)
            try? await backend.put("flark.json", data: data, precondition: .createOnly)
        }
    }
    ...
    public func getBlob(_ id: String) async throws -> Data {
        try await backend.get("blobs/\(id)").data
    }
    ...
    public func append(_ event: Event) async throws {
        ...
        let path = "events/\(identity.authorID)/\(e.hlc.description)-\(e.eventID).json"
    ...
    public func listEventPaths() async throws -> [String] {
        var paths: [String] = []
        for authorDir in try await backend.list("events") where authorDir.isDirectory {
    ...
    public func writeProfile(displayName: String, avatarBlobID: String?) async throws {
        ...
        try await backend.put("profiles/\(identity.authorID).json", data: data)
    }

    public func loadProfiles() async throws -> [String: ProfileFile] {
        var out: [String: ProfileFile] = [:]
        for f in try await backend.list("profiles")
        where !f.isDirectory && f.path.hasSuffix(".json") {
